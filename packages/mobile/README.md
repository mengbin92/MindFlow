...
├── editor/              # 编辑器桥接层
├── models/              # 数据模型
├── platform/            # 平台适配（文件系统 IO/Stub）
├── render/              # 预览渲染与扩展语法桥接
│   ├── syntax_bridge_service.dart   # 扩展语法占位符生成
│   ├── preview_render_service.dart  # HTML 文档编排 + CDN 注入
│   └── presentation_service.dart    # reveal.js HTML 生成
├── repositories/        # 仓储实现
├── services/            # 服务层（存储、导出、文件写入）
└── ui/                  # UI 层
    ├── screens/         # 页面
    │   ├── app_shell_screen.dart      # 主壳（三栏/导航栏）
    │   ├── editor_screen.dart         # 编辑器页面
    │   ├── presentation_screen.dart   # 演示模式
